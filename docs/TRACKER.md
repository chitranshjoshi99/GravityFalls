...
| # | Task | Spec | Verify | Status | Notes |
|---|---|---|---|---|---|
| 6.1 | `test_all.gd` fully green | all | Exit 0, every check above | ⬜ | **Exit 0 alone is not the gate.** Godot 4.7.1 exits **0** when `--script` fails to *parse* — the suite runs nothing and still looks green, and `--check-only` exits 0 too. Green means the run also printed `test_all: N checks passed`. Found at 0.9 |
